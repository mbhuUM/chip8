...
void chip8::emulateCycle() {
    //Fetch Opcode - Opcode is 2 bytes long 
    unsigned char firstPart = this->mem[this->PC];
    unsigned char secondPart = this->mem[this->PC + 1];

    //Decode Opcode and execute 
    unsigned short opcode = firstPart >> 8 | secondPart; 
    opcodeMapper(opcode);
    this->PC += 2;
    //Update Timers
...
void chip8::opcodeMapper(short opcode) {
     switch(opcode & 0xF000) { //this is to strip the first 4 bits

        //00E0 clear screen
        case 0x0000:
            chip8::clear_screen_00E0();
            break;
